...
function showPlatformView(view){
 ...
 if(view==="home"){refreshMenuCounts();}
...
async function startupCheck(manual=false){
 const expected="5.23.6",bs=$("startupBuildStatus"),bn=$("startupBuildNote"),rs=$("startupRunwayStatus"),rn=$("startupRunwayNote"),ls=$("startupLiveStatus"),ln=$("startupLiveNote"),notice=$("startupDataNotice"),btn=$("refreshDataStatusBtn");
...
window.addEventListener("pageshow",()=>{refreshMenuCounts();if(!openSharedToldFromHash()&&!window.currentMissionMeta)showPlatformView("home")});
window.addEventListener("load",()=>{refreshMenuCounts();if(!openSharedToldFromHash()&&!window.currentMissionMeta)showPlatformView("home")});
