...
function manualWeatherModeChanged(){
 ...
 if(mode==="LIVE"&&p){const m=(st?.live?.metar)||p.metar||{};const put=(id,x)=>{if($(id))$(id).value=x??""};put("manualWxOAT",firstVal(m,["temp_c","temperature_c","temp","temperature"]));put("manualWxPA",st?.live?.pressure_altitude_ft??p.pressure_altitude_ft??"");put("manualWxAlt",st?.live?.altimeter_inhg??p.altimeter_inhg??"");put("manualWxDir",firstVal(m,["wind_dir","wdir","windDir","wind_direction"]));put("manualWxSpeed",firstVal(m,["wind_kt","wspd","windSpeed","wind_speed_kt"]));put("manualWxGust",firstVal(m,["wind_gust_kt","wgst"]));}
...
function showPlatformView(view){
 const ids=["home","newtrip","ops","told","archive"];
 ids.forEach(v=>{
...
function saveCurrentDraft(){
 ...
 const e=$("draftSaveStatus");if(e)e.textContent=`Draft saved ${new Date(rec.updated_at).toLocaleTimeString([], {hour:'numeric',minute:'2-digit'})}`;
}
function scheduleDraftSave(){
 if(!window.currentMissionMeta||window.currentMissionMeta.completed)return;
 const e=$("draftSaveStatus");if(e)e.textContent="Saving draft…";
...
async function startupCheck(){
 const expected="5.22.7",bs=$("startupBuildStatus"),bn=$("startupBuildNote"),rs=$("startupRunwayStatus"),rn=$("startupRunwayNote"),ls=$("startupLiveStatus"),ln=$("startupLiveNote"),notice=$("startupDataNotice");
 try{
...
window.addEventListener("pageshow",()=>{if(!window.currentMissionMeta)showPlatformView("home")});
window.addEventListener("load",()=>{if(!window.currentMissionMeta)showPlatformView("home")});
