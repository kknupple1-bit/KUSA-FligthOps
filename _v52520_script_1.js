...
const F50_DIGITIZATION_STATUS={
  complete:[
    ...
    'S+Flaps 20° §5.45.6 BFL nomograph panels — PA/temperature, weight, runway-slope, wind and field-length axis directly digitized into source-controlled SHADOW dataset; 3/3 published anchors within 250 ft release-gate tolerance',
  ...
  pending:[
    ...
    'S+Flaps 20° BFL production activation / §5.45.7 V1 — direct §5.45.6 shadow engine passes published anchors; expanded independent-point regression still required before operational activation',
...
function f50ShadowWeightX(xIn,weightLb){
  ...
  const seeds=(n.weight_panel?.family_seed_x_px||[]).map(Number),rows=n.weight_panel?.rows_lb||{};
  const ws=Object.keys(rows).map(Number).filter(Number.isFinite).sort((a,b)=>a-b);
  if(!seeds.length||!ws.length||weightLb<ws[0]||weightLb>ws[ws.length-1])return null;
  const shift=Number(n.registration?.panel_x_registration_shift_px||0),xin=xIn+shift;
  const atW=w=>f50ShadowInterp(seeds,(rows[String(w)]||[]).map(Number),xin);
...
async function loadF50S20NomographShadow(){
  ...
  if(!F50_S20_NOMOGRAPH_SHADOW_PROMISE)F50_S20_NOMOGRAPH_SHADOW_PROMISE=fetch(apiUrl('/data/f50_s20_nomograph_rev31.json'),{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`S20 nomograph dataset HTTP ${r.status}`);return r.json()}).then(d=>{
    if(d?.schema!=='kusa.f50.s20.nomograph.v1'||d?.source?.chart?.indexOf('5.45.6')<0)throw new Error('S20 nomograph dataset integrity check failed');
...
function f50BflMigrationSource(cfg){
  if(cfg==="S20")return "DTM813 §5.45.6 BFL • direct panel digitization SHADOW regression PASS on published anchors • §5.45.7 V1 output-axis + published end-to-end trace digitization added in Phase 7 • continuous V1 panel solver remains validation-only pending expanded trace set";
...
async function startupCheck(manual=false){
 const expected="5.25.20",bs=$("startupBuildStatus"),bn=$("startupBuildNote"),ps=$("startupPerfStatus"),pn=$("startupPerfNote"),rs=$("startupRunwayStatus"),rn=$("startupRunwayNote"),ls=$("startupLiveStatus"),ln=$("startupLiveNote"),notice=$("startupDataNotice"),btn=$("refreshDataStatusBtn");
