...
note:(window.currentMissionMeta&&window.currentMissionMeta.note)||$("homeTripNote")?.value?.trim()||"",
   ...
   total_baggage_lb:Math.max(0,num("totalBag")),
...
if($("homeTripNote"))$("homeTripNote").value=s.note||record.note||"";
 ...
 if(Array.isArray(s.baggage)){
   ...
   // Keep the mission Total Baggage field and compartment loads synchronized on restore.
   syncTotalBagFromCompartments();
 } else if(Number.isFinite(Number(s.total_baggage_lb))){
   setBaggageFromTotal(Number(s.total_baggage_lb));
 }
...
function effectiveDepartureRunwayAssessment(){
 const published=departurePublishedLength();let state="OPEN",usable=published,source="PUBLISHED";
 if(departureNotams.status==="CHECKED"){state=departureNotams.runway_state||"OPEN";if(Number.isFinite(departureNotams.usable_length_ft))usable=departureNotams.usable_length_ft;source=departureNotams.source||"FAA NMS";}
 return{runway:selectedDepartureRunway(),published,usable,state,source};
}
...
async function refreshDepartureNotams(force=false){
 ...
 try{const res=await fetch(`/api/notams?icao=${encodeURIComponent(dep)}`),j=await res.json();if(!res.ok||!j.ok)throw new Error(j.message||"FAA NMS unavailable");
   const items=Array.isArray(j.items)?j.items:[],parsed=parseRunwayClosureFromNotams(items,r?r.runway_id:null);departureNotams={status:"CHECKED",source:j.source||"FAA NMS",items,runway_state:parsed.state,usable_length_ft:parsed.usable,checked_at:j.checked_at||new Date().toISOString()};
...
async function startupCheck(){
 const expected="5.22.9",bs=$("startupBuildStatus"),bn=$("startupBuildNote"),rs=$("startupRunwayStatus"),rn=$("startupRunwayNote"),ls=$("startupLiveStatus"),ln=$("startupLiveNote"),notice=$("startupDataNotice");
 try{
