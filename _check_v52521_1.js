...
const F50_DIGITIZATION_STATUS={
  complete:[
    ...
    'S+Flaps 20° §5.45.6 BFL nomograph panels — corrected REF-line weight-family traversal; published-anchor shadow regression PASS 4/4 within ±250 ft; expanded-envelope release testing still required',
  ...
  pending:[
    ...
    'S+Flaps 20° BFL production activation / §5.45.7 V1 — §5.45.6 corrected shadow engine passes 4/4 published anchors but remains non-operational pending expanded envelope; §5.45.7 continuous V1 solver remains pending',
    'Slats BFL / V1 / VMBE — FTA-PA-001019 §5-25 through §5-28',
    'Clean 1.5Vs — DTM813 §5.30.1/§5.30.2; N33AP pitot-probe / SB F50-197 applicability confirmation required',
    'S+Flaps 48° landing distance — DTM813 §5.60.4',
    'S+Flaps 20° landing distance — DTM813 §5.65.4',
    'Wet-runway takeoff BFL / V1 — DTM813 §5.46A and §5.51A',
...
function f50RunShadowRegression(){
  ...
  if(el){
    el.innerHTML=pass?`<b>SHADOW NOMOGRAPH:</b> PASS ${results.length}/${results.length} published anchors • max error ${Math.round(maxError)} ft • NOT YET RELEASED TO OPERATIONAL CALCULATION`:`<b>SHADOW NOMOGRAPH:</b> VALIDATION HOLD • ${valid.length}/${results.length} anchors computed • max error ${Number.isFinite(maxError)?Math.round(maxError)+' ft':'—'}`;
...
async function loadF50S20NomographShadow(){
  ...
  if(!F50_S20_NOMOGRAPH_SHADOW_PROMISE)F50_S20_NOMOGRAPH_SHADOW_PROMISE=fetch(apiUrl('/data/f50_s20_nomograph_rev31.json'),{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`S20 nomograph dataset HTTP ${r.status}`);return r.json()}).then(d=>{
    if(!['kusa.f50.s20.nomograph.v1','kusa.f50.s20.nomograph.v2'].includes(d?.schema)||d?.source?.chart?.indexOf('5.45.6')<0)throw new Error('S20 nomograph dataset integrity check failed');
    F50_S20_NOMOGRAPH_SHADOW=d;f50RunShadowRegression();return d;
  }).catch(e=>{F50_S20_NOMOGRAPH_SHADOW_PROMISE=null;const el=$('s20NomographShadowStatus');if(el){el.innerHTML='<b>SHADOW NOMOGRAPH:</b> DATASET UNAVAILABLE';el.className='warn'}throw e});
  return F50_S20_NOMOGRAPH_SHADOW_PROMISE;
}

function f50S20V1AxisKt(xPx){
  const a=F50_S20_V1_SHADOW?.ias_axis?.linear_fit;if(!a||!Number.isFinite(xPx))return null;
...
function f50RunV1ShadowRegression(){
  ...
  const results=anchors.map(a=>{const actual=f50S20V1AxisKt(Number(a.endpoint_x_px)),expected=Number(a.expected_v1_kt),error=Number.isFinite(actual)?actual-expected:null;return{id:a.id,expected,actual,error,pass:Number.isFinite(error)&&Math.abs(error)<=tol}});
  const valid=results.filter(r=>Number.isFinite(r.error)),maxError=valid.length?Math.max(...valid.map(r=>Math.abs(r.error))):Infinity;
  const pass=results.length>0&&valid.length===results.length&&results.every(r=>r.pass);
  F50_S20_V1_SHADOW_REGRESSION={pass,tolerance_kt:tol,max_abs_error_kt:maxError,results};
...
async function loadF50S20V1Shadow(){
  ...
  if(!F50_S20_V1_SHADOW_PROMISE)F50_S20_V1_SHADOW_PROMISE=fetch(apiUrl('/data/f50_s20_v1_nomograph_rev31.json'),{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`S20 V1 nomograph dataset HTTP ${r.status}`);return r.json()}).then(d=>{
    if(d?.schema!=='kusa.f50.s20.v1.nomograph.v1'||d?.source?.chart?.indexOf('5.45.7')<0)throw new Error('S20 V1 nomograph dataset integrity check failed');
...
function f50BflMigrationSource(cfg){
  if(cfg==="S20")return "DTM813 §5.45.6 BFL • expanded shadow regression HOLD after §5.45.12 34,000 lb anchor exposed weight-panel error • §5.45.7 V1 output-axis + published source anchors loaded • production grid unchanged";
  if(cfg==="SLATS")return "DTM813 §5.50.6/§5.50.7 BFL + §5.50.8 VMBE + §5.50.9 balanced-field V1 • direct nomograph migration in progress • published worked-example anchors loaded";
...
const F50_DASSAULT_SOURCE={
 ...
 landing:{S48:{vref:"DTM813 §5.60.5 — direct digitization complete",distance:"DTM813 §5.60.4 — digitization pending"},S20:{vref:"DTM813 §5.65.5 — direct digitization complete",distance:"DTM813 §5.65.4 — digitization pending"}}
};
...
function f50Dash4TakeoffN1(pa,tempC,antiIce="OFF"){
 ...
 const pas=kN(tab),pb=bracketN(pas,pa);if(!pb)return null;
 function atPA(p){
 ...
 if(a===b)return{...A,pa1:a,pa2:b,mode,source:F50_DASH4_N1_SOURCE[mode]};
 const f=(pa-a)/(b-a);
 return{side:lerpN(A.side,B.side,f),center:lerpN(A.center,B.center,f),pa1:a,pa2:b,mode,source:F50_DASH4_N1_SOURCE[mode]};
}
...
function currentF50TakeoffN1(){
 ...
 return{ok:true,text:`${n.side.toFixed(1)} / ${n.center.toFixed(1)}%`,side:n.side,center:n.center,source:`${n.source} • ${wxSource} • SAT ${temp.toFixed(1)}°C • PA ${Math.round(pa).toLocaleString()} ft`};
}
...
let F900B_QRH=null,F900B_LOAD_PROMISE=null,F900B_REFRESH_TIMER=null;
...
async function loadF900BData(){
 ...
 if(!F900B_LOAD_PROMISE)F900B_LOAD_PROMISE=fetch(apiUrl("/data/f900b_qrh_rev02.json"),{cache:"no-store"}).then(r=>{if(!r.ok)throw new Error(`900B dataset HTTP ${r.status}`);return r.json()}).then(d=>{
...
function f9GridValue(kind,filters,pa,temp,weight,key,transform){
 ...
 const pb=f9ArrayBracket(tables.map(t=>t.pressure_altitude_ft),pa);if(!pb)return null;
 ...
 if(values.some(v=>v==null))return null;
 return pb[0]===pb[1]?values[0]:lerpN(values[0],values[1],(pa-pb[0])/(pb[1]-pb[0]));
}
...
function f9ExactLimitCode(filters,pa,temp,weight){
 ...
 const table=F900B_QRH?.takeoff.find(t=>t.pressure_altitude_ft===pa&&Object.entries(filters).every(([k,v])=>String(t[k])===String(v)));
...
function f9TakeoffResult(){
 ...
 const out={pa,temp,weight,runway,filters};keys.forEach(key=>out[key]=f9GridValue("takeoff",filters,pa,temp,weight,key));
...
function f9LandingResult(pa,temp,weight,antiIce,runway){
 ...
 const out={pa,temp,weight,runway,antiIce};keys.forEach(key=>out[key]=f9GridValue("landing",filters,pa,temp,weight,key));
 out.vref_kt=f9Speed(F900B_QRH?.vref,weight,"vref_kt");
 return out;
}
function f9SourceLockText(pa,temp,weight){
 if(![pa,temp,weight].every(Number.isFinite))return"ENTER PRESSURE ALTITUDE, OAT AND WEIGHT";
 if(pa<0||pa>6000)return"SOURCE LOCKED — PA OUTSIDE 0–6,000 FT";
 if(weight<28000||weight>46500)return"SOURCE LOCKED — WEIGHT OUTSIDE 28,000–46,500 LB";
 return"SOURCE LOCKED — SELECTED POINT CROSSES AN UNPUBLISHED CELL";
}
function renderF900BTakeoff(out){
 f9Put("f9V1",f9Fmt(out.v1_kt,1," kt"));f9Put("f9VR",f9Fmt(out.vr_v2_kt,1," kt"));f9Put("f9VFT",f9Fmt(out.vft_kt,1," kt"));f9Put("f9VFR",f9Fmt(out.vfr_kt,1," kt"));
 ...
 if(Number.isFinite(out.weight)&&out.weight>45500){status="NO-GO • OVER STRUCTURAL MTOW";tone="bad"}
 else if(!fieldReady)status=f9SourceLockText(out.pa,out.temp,out.weight);
 ...
 f9Put("f9ToSource",`Falcon 900B QRH1 Rev02 • DGAC18DSOF182A • 50-10 • ${out.filters.flaps_deg}° F+S • A/I ${out.filters.anti_ice} • ${out.filters.surface} • bounded interpolation • wind 0 kt • slope 0%`);
}
function renderF900BLanding(prefix,out,factor,structuralGate=true){
 ...
 if(prefix==="f9Emer"){
   f9Put("f9EmerWeight",f9Fmt(out.weight,0," lb"));f9Put("f9EmerVref",f9Fmt(out.vref_kt,1," kt"));f9Put("f9EmerLD",f9Fmt(out.landing_distance_ft,0," ft"));f9Put("f9EmerRequired",f9Fmt(required,0," ft"));f9Put("f9EmerAvailable",f9Fmt(out.runway,0," ft"));f9Put("f9EmerMargin",f9Fmt(margin,0," ft"));f9Put("f9EmerACG",f9Fmt(out.approach_climb_gradient_pct,2,"%"));f9Put("f9EmerLCG",f9Fmt(out.landing_climb_gradient_pct,2,"%"));
 }else{
   f9Put("f9LdgVref",f9Fmt(out.vref_kt,1," kt"));f9Put("f9LdgLD",f9Fmt(out.landing_distance_ft,0," ft"));f9Put("f9Ldg167",f9Fmt(out.landing_distance_x_1_67_ft,0," ft"));f9Put("f9Ldg192",f9Fmt(out.landing_distance_x_1_92_ft,0," ft"));f9Put("f9LdgRequired",f9Fmt(required,0," ft"));f9Put("f9LdgMargin",f9Fmt(margin,0," ft"));f9Put("f9LdgACG",f9Fmt(out.approach_climb_gradient_pct,2,"%"));f9Put("f9LdgLCG",f9Fmt(out.landing_climb_gradient_pct,2,"%"));
 }
 const ready=[out.vref_kt,required,out.approach_climb_gradient_pct,out.landing_climb_gradient_pct].every(Number.isFinite);
 let status,tone="warn";
 if(structuralGate&&Number.isFinite(out.weight)&&out.weight>42000){status="NO-GO • OVER 42,000 LB MLW";tone="bad"}
 else if(!ready)status=f9SourceLockText(out.pa,out.temp,out.weight);
 else if(!Number.isFinite(out.runway))status="RUNWAY REQUIRED";
...
function syncF900BMissionToTOLD(){
 const pairs=[
  ...
  ["f9mToRunway","f9ToRunway"],["f9mToPA","f9ToPA"],["f9mToTemp","f9ToTemp"],["f9mToWeight","f9ToWeight"],
  ["f9mReturnBurn","f9ReturnBurn"],["f9mLdgAI","f9LdgAI"],["f9mLdgPA","f9LdgPA"],["f9mLdgTemp","f9LdgTemp"],
  ["f9mLdgWeight","f9LdgWeight"],["f9mLdgRunway","f9LdgRunway"],["f9mLdgFactor","f9LdgFactor"]
 ];
...
function syncF900BTOLDToMission(){
 const pairs=[
  ...
  ["f9ToRunway","f9mToRunway"],["f9ToPA","f9mToPA"],["f9ToTemp","f9mToTemp"],["f9ToWeight","f9mToWeight"],
  ["f9ReturnBurn","f9mReturnBurn"],["f9LdgAI","f9mLdgAI"],["f9LdgPA","f9mLdgPA"],["f9LdgTemp","f9mLdgTemp"],
  ["f9LdgWeight","f9mLdgWeight"],["f9LdgRunway","f9mLdgRunway"],["f9LdgFactor","f9mLdgFactor"]
 ];
...
async function refreshF900B(){
 try{await loadF900BData()}catch(e){f9Status("f9ToStatus","900B DATASET UNAVAILABLE","bad");f9Put("f9ToSource",String(e.message||e));return}
 ...
 const ldg=f9LandingResult(f9Num("f9LdgPA"),f9Num("f9LdgTemp"),f9Num("f9LdgWeight"),$("f9LdgAI")?.value||"OFF",f9Num("f9LdgRunway"));
 const factor=Number($("f9LdgFactor")?.value||1.67);renderF900BLanding("f9Ldg",ldg,factor,true);
 f9Put("f9LdgSource",`Falcon 900B QRH1 Rev02 • DGAC18DSOF182A • 50-25 / 50-28 / 50-29 • 40° F+S • A/I ${ldg.antiIce} • comparison LD × ${factor}`);
}
function scheduleF900BRefresh(){clearTimeout(F900B_REFRESH_TIMER);F900B_REFRESH_TIMER=setTimeout(refreshF900B,120)}
async function startF900BTrip(dep,dest,alt,tripDate=localISODate(new Date())){
 window.currentMissionMeta={id:newMissionId(),aircraft:"F900B",note:$("homeTripNote")?.value?.trim()||"",trip_date:tripDate,started_at:new Date().toISOString(),completed:false}; window.currentMissionMeta.trip_number=allocateTripNumber(window.currentMissionMeta.trip_date);
 if($("dep"))$("dep").value=dep;if($("dest"))$("dest").value=dest;if($("alt"))$("alt").value=alt||"";
 f9Put("f9Route",`${dep} → ${dest}${alt?` • Alternate ${alt}`:""}`);
 f9Put("f9mRoute",`${dep} → ${dest}${alt?` • Alternate ${alt}`:""}`);
...
async function refreshToldOperationalData(){
 if(window.currentMissionMeta?.aircraft==="F900B"){await refreshF900B();scheduleDraftSave();return;}
...
function showPlatformView(view){
 ...
 if(actualView==="f900bmission"){syncF900BTOLDToMission();refreshF900BMission();}
...
function captureF900BPlanState(){
 ...
 return {
   dep,dest,alt,aircraft:"F900B",note:val("f9mNote").trim(),trip_date:currentTripDate(),
   f900b:{
     ...
     landing_runway_ft:val("f9mLdgRunway"),landing_factor:val("f9mLdgFactor"),emergency_anti_ice:val("f9EmerAI")
   }
 };
}
function applyF900BPlanState(state){
 ...
 set("f9mLdgRunway",f.landing_runway_ft);set("f9mLdgFactor",f.landing_factor||"1.67");set("f9EmerAI",f.emergency_anti_ice||"OFF");
...
function f900bDraftRecord(){
 ...
 return {id:window.currentMissionMeta.id,trip_number:currentTripNumber(),trip_date:currentTripDate(),leg_number:window.currentMissionMeta.leg_number||1,aircraft:"F900B",note:state.note||window.currentMissionMeta.note||"",started_at:window.currentMissionMeta.started_at||new Date().toISOString(),updated_at:new Date().toISOString(),dep:state.dep,dest:state.dest,alt:state.alt,state};
}
async function restoreF900BFlightPlan(record,opts={}){
 ...
 window.currentMissionMeta={id:newId,trip_number:record.trip_number||allocateTripNumber(record.trip_date||state.trip_date||(opts.newCopy?localISODate(new Date()):(record.started_at||new Date()))),trip_date:record.trip_date||state.trip_date||localISODate(new Date()),leg_number:record.leg_number||1,aircraft:"F900B",note:state.note||record.note||"",started_at:opts.newCopy?new Date().toISOString():(record.started_at||new Date().toISOString()),completed:false,reopened_from_draft:!!opts.fromDraft,imported_editable_copy:!!opts.newCopy};
...
function startNewTrip(){
 ...
 if(aircraft==="F900B"){
   startF900BTrip(dep,dest,alt,tripDate).catch(e=>alert(`Falcon 900B dataset could not be loaded: ${e.message||e}`));
   return;
 }
...
function currentF900BSummaryText(){
 ...
 return [
  "KUSA FLIGHTOPS — FALCON 900B TOLD",
...
function buildF900BArchiveRecord(){
 ...
 return {id:window.currentMissionMeta?.id||newMissionId(),trip_number:currentTripNumber(),trip_date:currentTripDate(),leg_number:window.currentMissionMeta?.leg_number||1,aircraft:"F900B",note:state.note||window.currentMissionMeta?.note||"",started_at:window.currentMissionMeta?.started_at||null,completed_at:new Date().toISOString(),dep:state.dep,dest:state.dest,alt:state.alt,tow_lb:Number(f.takeoff_weight_lb)||null,ldw_lb:Number(f.landing_weight_lb)||null,to_status:textOf("f9ToStatus"),ldg_status:textOf("f9LdgStatus"),summary:currentF900BSummaryText(),state};
}
...
async function completeMission(){
 ...
 if(isF900B){
   syncF900BMissionToTOLD();await refreshF900B();
   ...
   return `<div class="archive-item">
     ...
     <div class="archive-head">
       <div>
         ...
         <div class="archive-meta">Trip ${td} • Leg ${esc(m.leg_number||1)} • archived ${dt} • ${m.aircraft||"N33AP"}${m.note?" • "+esc(m.note):""}</div>
       </div>
...
function updateToldCard(){
 ...
 setToldVspeed("toldVref",$("landVref").value?Number($("landVref").value).toFixed(1)+" kt":"SOURCE LOCKED");
 ...
 $("opTakeoffStatus").className=`told-op-status ${textOf("toStatus").includes("PASS")?"ok":textOf("toStatus").includes("NO-GO")?"bad":"warn"}`;
 ...
 const emergencyPerf=Number.isFinite(emergencyWeight)&&Number.isFinite(emergencyPA)?fsLanding("S48",emergencyPA,emergencyWeight/1000):null;
 ...
 else if(!Number.isFinite(emergencyRequired))emergencyStatus="SOURCE LOCKED";
 ...
 setToldVspeed("opEmerVref",emergencyPerf&&Number.isFinite(emergencyPerf.speed)?`${emergencyPerf.speed.toFixed(1)} kt`:textOf("vsVREF"));
 ...
 $("opEmerPA").textContent=Number.isFinite(emergencyPA)?`${Math.round(emergencyPA).toLocaleString()} ft`:"—";
 $("opEmerSource").textContent=emergencyPerf?`VREF: ${DTM813_S48_VREF_SOURCE} • Landing distance: DTM813 §5.60.4 legacy grid pending direct digitization`:"SOURCE LOCKED";
 ...
 setToldVspeed("opLandingVref",$("landVref").value?`${Number($("landVref").value).toFixed(1)} kt`:"SOURCE LOCKED");
 ...
 $("opLandingStatus").className=`told-op-status ${textOf("landStatus").includes("PASS")?"ok":textOf("landStatus").includes("NO-GO")?"bad":"warn"}`;
 ...
 if(obsEff.state==="CONFLICT")issues.push("PILOT OBSTACLE CONFLICT RECORDED");
   ...
   else issues.push("AUTO OBSTACLE NOTAM REVIEW REQUIRED");
 }
 ...
 else if(ts.includes("PERFORMANCE PASS")&&!obsEff.cleared)issues.push("TAKEOFF PERFORMANCE EVALUATED • AUTO OBSTACLE REVIEW INCOMPLETE");
 if(textOf("landStatus").includes("PENDING")||textOf("landStatus").includes("LOCKED"))issues.push("LANDING LIMITS NOT FULLY EVALUATED");
 $("toldOverall").textContent=issues.length?issues.join(" • "):"TOLD OPERATIONAL SUMMARY CURRENT — review all values before use.";
...
function takeoffComparisonFor(cfg){
 ...
 if(condition==="WET")reason="WET DATA SOURCE LOCKED";
...
function updateTakeoffComparison(){
 ...
 entries.forEach(({cfg,prefix,card,button})=>{
   ...
   $(`${prefix}Speeds`).textContent=`${Number.isFinite(x.v1)?x.v1.toFixed(1):"—"} / ${Number.isFinite(x.vr)?x.vr.toFixed(1):"—"} kt`;
...
async function startupCheck(manual=false){
 const expected="5.25.21",bs=$("startupBuildStatus"),bn=$("startupBuildNote"),ps=$("startupPerfStatus"),pn=$("startupPerfNote"),rs=$("startupRunwayStatus"),rn=$("startupRunwayNote"),ls=$("startupLiveStatus"),ln=$("startupLiveNote"),notice=$("startupDataNotice"),btn=$("refreshDataStatusBtn");
 if(btn)btn.disabled=true;
 if(bs){bs.textContent="CHECKING";bs.className="warn"} if(bn)bn.textContent="Verifying app/server build…";
 if(ps){ps.textContent="CHECKING";ps.className="warn"} if(pn)pn.textContent="Verifying N33AP + Falcon 900B datasets…";
 ...
 try{
   ...
   try{
     const f9=await loadF900BData();
     const perfOk=f9?.takeoff?.length===32&&f9?.landing?.length===8;
     if(ps){ps.textContent=perfOk?"LOADED":"DATA MISSING";ps.className=perfOk?"ok":"bad"}
     if(pn)pn.textContent=perfOk?"N33AP AFM/AFMS + Falcon 900B QRH1 Rev02 • 32 takeoff / 8 landing tables":"Falcon 900B dataset failed integrity check";
   }catch(perfError){if(ps){ps.textContent="900B DATA MISSING";ps.className="bad"}if(pn)pn.textContent="N33AP remains available; redeploy the Falcon 900B dataset."}
   try{
...
["f9ToPA","f9ToTemp","f9ToWeight","f9ToRunway","f9ReturnBurn","f9LdgPA","f9LdgTemp","f9LdgWeight","f9LdgRunway"].forEach(id=>$(id)?.addEventListener("input",scheduleF900BRefresh));
["f9mToPA","f9mToTemp","f9mToWeight","f9mToRunway","f9mReturnBurn","f9mLdgPA","f9mLdgTemp","f9mLdgWeight","f9mLdgRunway"].forEach(id=>$(id)?.addEventListener("input",()=>{clearTimeout(F900B_REFRESH_TIMER);F900B_REFRESH_TIMER=setTimeout(refreshF900BMission,120)}));
["f9mToConfig","f9mToAI","f9mToSurface","f9mLdgAI","f9mLdgFactor"].forEach(id=>$(id)?.addEventListener("change",refreshF900BMission));
["f9ToConfig","f9ToAI","f9ToSurface","f9EmerAI","f9LdgAI","f9LdgFactor"].forEach(id=>$(id)?.addEventListener("change",refreshF900B));
