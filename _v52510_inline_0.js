...
async function startF900BTrip(dep,dest,alt){
 window.currentMissionMeta={id:newMissionId(),aircraft:"F900B",note:$("homeTripNote")?.value?.trim()||"",started_at:new Date().toISOString(),completed:false}; window.currentMissionMeta.trip_number=allocateTripNumber(window.currentMissionMeta.started_at);
...
function tripDateKey(value=new Date()){
...
function currentTripNumber(){
 ...
 if(!window.currentMissionMeta.trip_number)window.currentMissionMeta.trip_number=allocateTripNumber(window.currentMissionMeta.started_at||new Date());
...
function captureF900BPlanState(){
 ...
 return {
   dep,dest,alt,aircraft:"F900B",note:val("f9mNote").trim(),
   f900b:{
...
async function restoreF900BFlightPlan(record,opts={}){
 ...
 window.currentMissionMeta={id:newId,trip_number:record.trip_number||allocateTripNumber(opts.newCopy?new Date():(record.started_at||new Date())),leg_number:record.leg_number||1,aircraft:"F900B",note:state.note||record.note||"",started_at:opts.newCopy?new Date().toISOString():(record.started_at||new Date().toISOString()),completed:false,reopened_from_draft:!!opts.fromDraft,imported_editable_copy:!!opts.newCopy};
...
function startNewTrip(){
 ...
 if(aircraft==="F900B"){
   startF900BTrip(dep,dest,alt).catch(e=>alert(`Falcon 900B dataset could not be loaded: ${e.message||e}`));
   return;
 }
...
note:$("homeTripNote").value.trim(),
 ...
 window.currentMissionMeta.trip_number=allocateTripNumber(window.currentMissionMeta.started_at);
 recalculate();
 showPlatformView("ops");
 saveCurrentDraft();
...
function editableTripShareLabel(record){
 ...
 const d=new Date(r.started_at||r.updated_at||Date.now());
 const date=Number.isNaN(d.getTime())?new Date().toLocaleDateString("en-US"):`${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}-${d.getFullYear()}`;
...
if($("homeTripNote"))$("homeTripNote").value=s.note||record.note||"";
 ...
 window.currentMissionMeta={
   ...
   trip_number:record.trip_number||allocateTripNumber(record.started_at||new Date()),
...
function buildArchiveRecord(){
 ...
 return{
   ...
   trip_number:currentTripNumber(),leg_number:window.currentMissionMeta?.leg_number||1,
...
async function openDraftFlightPlan(i){
 ...
 window.currentMissionMeta={id:m.id||newMissionId(),trip_number:m.trip_number||allocateTripNumber(m.started_at||new Date()),leg_number:m.leg_number||1,aircraft:m.aircraft||"N33AP",note:m.note||"",started_at:m.started_at||new Date().toISOString(),completed:false,reopened_from_draft:true};
   ...
   return `<div class="archive-item">
     ...
     <div class="archive-head">
       <div>
         ...
         <div class="archive-meta">Leg ${esc(m.leg_number||1)} • ${dt} • ${m.aircraft||"N33AP"}${m.note?" • "+esc(m.note):""}</div>
       </div>
...
function duplicateArchiveFlightPlan(i){
 ...
 copy.trip_number=allocateTripNumber(new Date());copy.leg_number=1;
...
function updateToldCard(){
 ...
 $("toldRoute").textContent=`Trip ${currentTripNumber()||"—"} • ${depCode} → ${destCode}${perfIsManual()?" • MANUAL PERFORMANCE PLAN":""}`;
 ...
 $("opV1").textContent=textOf("vsV1");
 $("opVR").textContent=textOf("vsVR");
 $("opVfr").textContent=textOf("vsVFT");
 $("op15Vs").textContent=textOf("vsVFS");
 ...
 $("opEmerVref").textContent=emergencyPerf&&Number.isFinite(emergencyPerf.speed)?`${emergencyPerf.speed.toFixed(1)} kt`:textOf("vsVREF");
 ...
 $("opEmerSource").textContent=emergencyPerf?"FlightSafety S+48 • departure PA":"SOURCE LOCKED";
 ...
 $("opLandingVref").textContent=$("landVref").value?`${Number($("landVref").value).toFixed(1)} kt`:"SOURCE LOCKED";
...
function takeoffComparisonFor(cfg){
 ...
 let result=null,source="PUBLISHED GRID",reason="";
...
async function startupCheck(manual=false){
 const expected="5.25.10",bs=$("startupBuildStatus"),bn=$("startupBuildNote"),ps=$("startupPerfStatus"),pn=$("startupPerfNote"),rs=$("startupRunwayStatus"),rn=$("startupRunwayNote"),ls=$("startupLiveStatus"),ln=$("startupLiveNote"),notice=$("startupDataNotice"),btn=$("refreshDataStatusBtn");
