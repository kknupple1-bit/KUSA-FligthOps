...
function f9DrawCGTrace(wb){
 ...
 const px=v=>45+(v-minCG)/(maxCG-minCG)*(W-70),py=v=>25+(v-minW)/(maxW-minW)*(H-60);
 ...
 x.save();x.fillStyle="rgba(92,146,190,.12)";x.beginPath();fwd.forEach((p,i)=>i?x.lineTo(px(p[1]),py(p[0])):x.moveTo(px(p[1]),py(p[0])));[...aft].reverse().forEach(p=>x.lineTo(px(p[1]),py(p[0])));x.closePath();x.fill();x.restore();
 const limitColor="#86a9c7";x.strokeStyle=limitColor;x.lineWidth=2;x.beginPath();fwd.forEach((p,i)=>i?x.lineTo(px(p[1]),py(p[0])):x.moveTo(px(p[1]),py(p[0])));x.stroke();
 x.strokeStyle=limitColor;x.beginPath();aft.forEach((p,i)=>i?x.lineTo(px(p[1]),py(p[0])):x.moveTo(px(p[1]),py(p[0])));x.stroke();
 x.fillStyle="#b8cadc";x.font="bold 10px Arial";const hi=env[env.length-1];if(hi){x.fillText("FWD CG LIMIT",Math.max(42,px(hi[1])-82),Math.max(22,py(hi[0])-8));x.fillText("AFT CG LIMIT",Math.min(W-92,px(hi[2])+8),Math.max(22,py(hi[0])-8));}
 const refs=[[lim.mzfw_lb,"MZFW"],[lim.mlw_lb,"MLW"],[lim.mtow_lb,"MTOW"],[lim.mrw_lb,"MRW"]];
 refs.forEach(([w,label],i)=>{if(!Number.isFinite(w))return;x.save();x.strokeStyle=i<2?"#7f96ad":"#ff6b6b";x.lineWidth=1.4;x.setLineDash([7,5]);x.beginPath();x.moveTo(40,py(w));x.lineTo(W-20,py(w));x.stroke();x.setLineDash([]);x.fillStyle=i<2?"#b8cadc":"#ffb0b0";x.font="bold 10px Arial";x.fillText(`${label} ${Number(w).toLocaleString()}`,Math.max(45,W-145),py(w)-4);x.restore();});
 if(wb.tcg&&wb.lcg&&Number.isFinite(wb.tow)&&Number.isFinite(wb.ldw)){x.save();x.strokeStyle="#8fd3ff";x.lineWidth=3;x.setLineDash([8,5]);x.beginPath();x.moveTo(px(wb.tcg.pct),py(wb.tow));x.lineTo(px(wb.lcg.pct),py(wb.ldw));x.stroke();x.setLineDash([]);x.restore();}
 const point=(w,cg,label,color,r=6)=>{if(!Number.isFinite(w)||!cg||!Number.isFinite(cg.pct))return;x.fillStyle=color;x.beginPath();x.arc(px(cg.pct),py(w),r,0,Math.PI*2);x.fill();x.fillStyle="#d6e7f8";x.font="bold 10px Arial";x.fillText(label,px(cg.pct)+8,py(w)-7);};
 point(wb.zfw,wb.zcg,"ZFW","#edf5ff",4);point(wb.ramp,wb.rcg,"RAMP","#d9a6ff",5);point(wb.tow,wb.tcg,"TO","#8fd3ff",7);point(wb.ldw,wb.lcg,"LDG","#55d18b",7);
...
async function f9RecalcWB(){
 ...
 const flightPathOK=Array.isArray(burnPath)&&burnPath.length>1&&burnPath.every(p=>inside(p.weight,p.cg));
 ...
 window.f900bWBLatest={zfw,ramp,tow,ldw,fob,toFuel,ldgFuel,zcg,rcg,tcg,lcg,structuralOK,envOK,loadOK,ok,burnPath};f9DrawCGTrace(window.f900bWBLatest);f9UpdateOperationalGate();f9RenderMissionParity();scheduleDraftSave();return window.f900bWBLatest;
}
...
function renderF900BTakeoff(out){
 ...
 f9Put("f9GCLB2",f9Fmt(out.second_segment_gross_gradient_pct,2,"%"));f9Put("f9Accel",f9Fmt(out.acceleration_g,2," G"));f9Put("f9Pitch",f9Fmt(out.pitch_deg,1,"°"));f9Put("f9Limit",out.limit);
...
function f9RenderToldParity(){
 ...
 f9Put("f9SupportTakeoffSource",textOf("f9ToSource"));f9Put("f9SupportLandingDetail",`LD ${textOf("f9LdgLD")} • required ${textOf("f9LdgRequired")} • margin ${textOf("f9LdgMargin")} • ${textOf("f9LdgStatus")}`);f9Put("f9SupportLandingSource",textOf("f9LdgSource"));
}
...
async function startupCheck(manual=false){
 const expected="5.25.57",bs=$("startupBuildStatus"),bn=$("startupBuildNote"),ps=$("startupPerfStatus"),pn=$("startupPerfNote"),rs=$("startupRunwayStatus"),rn=$("startupRunwayNote"),ls=$("startupLiveStatus"),ln=$("startupLiveNote"),notice=$("startupDataNotice"),btn=$("refreshDataStatusBtn");
