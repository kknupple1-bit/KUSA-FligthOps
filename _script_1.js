...
let departureObstacleReview={status:"NOT_CHECKED",airport:null,items:[],checked_at:null,message:"FAA NMS departure NOTAM scan only — not a clearance analysis."};
let manualWeather={
 dep:{mode:"LIVE",applied:false,entered_at:null,note:"",live:null},
 dest:{mode:"LIVE",applied:false,entered_at:null,note:"",live:null}
};
...
function loadManualWeatherForm(){
 ...
 val("manualWxDir",firstVal(m,["wind_dir","wdir","windDir","wind_direction"])); val("manualWxSpeed",firstVal(m,["wind_kt","wspd","windSpeed","wind_speed_kt"])); val("manualWxGust",firstVal(m,["wind_gust_kt","wgst"]));
 val("manualWxNote",st?.note||""); manualWeatherModeChanged(); updateWeatherSourceBadges();
}
function manualWeatherModeChanged(){
 const mode=$("manualWxMode")?.value||"LIVE",ids=["manualWxOAT","manualWxPA","manualWxAlt","manualWxDir","manualWxSpeed","manualWxGust","manualWxNote"];ids.forEach(id=>{if($(id))$(id).disabled=mode!=="MANUAL"});
 const s=$("manualWxStatus");if(s)s.textContent=mode==="MANUAL"?"Enter pilot-observed/known weather and press APPLY MANUAL WEATHER.":"LIVE / reported weather selected. Apply is not required.";
}
function applyManualWeather(){
 ...
 if(mode==="LIVE"){const L=st.live||{};p.metar=L.metar||null;p.pressure_altitude_ft=L.pressure_altitude_ft??null;p.density_altitude_ft=L.density_altitude_ft??null;p.altimeter_inhg=L.altimeter_inhg??null;st.mode="LIVE";st.applied=false;st.entered_at=null;st.note="";}
 else{
   ...
   const now=new Date().toISOString();p.metar={icao:p.icao||p.airport?.ident||"",raw:"MANUAL WEATHER ENTRY",obs_time:now,temp_c:oat,wind_dir:Number.isFinite(dir)?dir:null,wind_kt:Number.isFinite(spd)?spd:null,wind_gust_kt:Number.isFinite(gust)?gust:null,manual:true};p.pressure_altitude_ft=Math.round(pa);p.altimeter_inhg=Number.isFinite(alt)?alt:null;const isa=15-0.0019812*pa;p.density_altitude_ft=Math.round(pa+120*(oat-isa));st.mode="MANUAL";st.applied=true;st.entered_at=now;st.note=String($("manualWxNote")?.value||"").trim();
 }
 ...
 const ms=$("manualWxStatus");if(ms)ms.textContent=weatherSourceState(which).mode==="MANUAL"?`MANUAL WEATHER APPLIED ${new Date(manualWeather[which].entered_at).toLocaleString()}`:"LIVE weather restored.";
}
...
function resetMissionForNewTrip(){
 ...
 missionData=null;
 manualWeather={dep:{mode:"LIVE",applied:false,entered_at:null,note:"",live:null},dest:{mode:"LIVE",applied:false,entered_at:null,note:"",live:null}};
...
function currentMissionSummaryText(){
 ...
 const lines=[
   ...
   `Climb-Limited TOW: ${$("toClimbWt")?.value?Number($("toClimbWt").value).toLocaleString()+" lb":"SOURCE LOCKED"}`,
   ...
   `Obstacle Analysis: ${$("obstacleStatus")?$("obstacleStatus").value||textOf("obstacleStatus"):"Pending / not checked"}`,
   "",
   "=== LANDING TOLD ===",
...
async function shareCurrentMission(){
 ...
 const title=`KUSA FlightOps ${cleanICAO($("dep").value)}-${cleanICAO($("dest").value)}`;
 try{
   if(navigator.share){await navigator.share({title,text});return}
   await navigator.clipboard.writeText(text);
   alert("Flight plan TOLD copied to clipboard.");
 }catch(e){
   if(e&&e.name!=="AbortError")alert("Unable to share automatically. Use Email or Print.");
 }
}
function emailCurrentMission(){
 const subject=encodeURIComponent(`KUSA FlightOps TOLD — ${cleanICAO($("dep").value)} to ${cleanICAO($("dest").value)} — N33AP`);
 const body=encodeURIComponent(currentMissionSummaryText());
...
if($("homeTripNote"))$("homeTripNote").value=s.note||record.note||"";
 ...
 if(Array.isArray(s.baggage)){
   ["bagComp0","bagComp1","bagComp2"].forEach((id,i)=>{
     if($(id)&&Number.isFinite(Number(s.baggage[i])))$(id).value=s.baggage[i];
   });
 }
...
function completeMission(){
 ...
 if(window.currentMissionMeta)window.currentMissionMeta.completed=true;
 renderArchive();
...
async function shareArchiveMission(i){
 ...
 try{
   if(navigator.share){await navigator.share({title:`KUSA FlightOps ${m.dep}-${m.dest}`,text:m.summary});return}
   await navigator.clipboard.writeText(m.summary);alert("Completed flight plan TOLD copied to clipboard.");
 }catch(e){}
}
function emailArchiveMission(i){
 const m=getMissionArchive()[i];if(!m)return;
 location.href=`mailto:?subject=${encodeURIComponent("KUSA FlightOps TOLD — "+m.dep+" to "+m.dest+" — "+(m.aircraft||"N33AP"))}&body=${encodeURIComponent(m.summary)}`;
}
...
function updateToldCard(){
 ...
 $("toldDepSurface").textContent=`${perfIsManual()&&perfTakeoffRunwayLen()?Number(perfTakeoffRunwayLen()).toLocaleString()+" ft":(dr&&dr.length_ft?Number(dr.length_ft).toLocaleString()+" ft":"—")} / ${$("toRunwayCondition")?$("toRunwayCondition").value:"DRY"}`;
 ...
 if($("toldClimbTOW"))$("toldClimbTOW").textContent=$("toClimbWt")?.value?Number($("toClimbWt").value).toLocaleString()+" lb":"SOURCE LOCKED";
 ...
 else if(obs.status==="REVIEWED")issues.push("AUTO OBSTACLE NOTAM REVIEW COMPLETE");
 else issues.push("AUTO OBSTACLE NOTAM REVIEW REQUIRED");
 issues.push($("perfObs")?.value==="true"?"PILOT OBSTACLE CLEARANCE REVIEW RECORDED":"OBSTACLE CLEARANCE ANALYSIS NOT PROVIDED");
 ...
 if(ts.includes("PENDING")||ts.includes("LOCKED"))issues.push("TAKEOFF PERFORMANCE NOT FULLY EVALUATED");
 else if(ts.includes("PERFORMANCE PASS"))issues.push("TAKEOFF PERFORMANCE EVALUATED • OBSTACLE CLEARANCE SEPARATE");
...
function drawTrace(zfwW,zfwCG,path,toW,toCG,ldW,ldCG){
 ...
 const dpr=devicePixelRatio||1,W=c.clientWidth,H=320;c.width=W*dpr;c.height=H*dpr;
...
function syncTakeoffSection(){
 ...
 pending.push("OBSTACLE CLEARANCE ANALYSIS NOT PROVIDED");
 if($("perfObs").value==="false")pending.push("MANUAL OBSTACLE CHECK NOT CLEARED");
...
function recalculate(){
 ...
 const capt=num("captWt"),fo=num("foWt"),crew=capt+fo; const bagLoads=baggageLoadsFromUI(); const bag=bagLoads.reduce((a,b)=>a+b,0);
...
function selectedRunwayChanged(){
 currentNotams={status:"NOT_CHECKED",source:"NONE",items:[],runway_state:null,usable_length_ft:null,checked_at:null};
 departureObstacleReview={status:"NOT_CHECKED",airport:null,items:[],checked_at:null,message:"FAA NMS departure NOTAM scan only — not a clearance analysis."};
...
async function loadMission(){
 ...
 $("dep").value=dep;$("dest").value=dest;$("alt").value=alt;$("missionStatus").textContent="Loading airport and weather data...";
 try{
   ...
   setTimeout(()=>{refreshNotams().catch(()=>{});},150);
   $("missionStatus").innerHTML=depCount||destCount?`<span class="ok">Mission loaded.</span> Runway data: ${dep} ${depCount} (${depSrc}) • ${dest} ${destCount} (${destSrc}). First available runway auto-selected.`:`<span class="warn">Weather request completed, but no runway data was returned from the packaged cache or the nationwide FAA NASR-derived lookup for ${dep} or ${dest}. Use the manual runway fallback and verify current airport data.</span>`;
 }catch(e){$("missionStatus").innerHTML='<span class="bad">'+e+'</span>'}
}
...
function parseRunwayClosureFromNotams(items, runwayId){
 const rawId=String(runwayId||"");
 const ids=[rawId,rawId.replace(/^0/,"")].filter(Boolean);
 let state="OPEN",usable=null,matched=[];
 for(const n of items||[]){
   const raw=String(n.raw||n.text||n.message||n||"").toUpperCase();
   if(!raw)continue;
   const runwayMention=/\bRWY\b|\bRUNWAY\b/.test(raw);
   const idHit=!ids.length||ids.some(id=>raw.includes(`RWY ${id}`)||raw.includes(`RUNWAY ${id}`));
   if(runwayMention&&idHit){
     matched.push(raw);
     if(/\bCLSD\b|\bCLOSED\b/.test(raw))state="CLOSED";
     else if(state!=="CLOSED")state="RESTRICTED";
     for(const p of [
       /(?:LDA|USABLE|AVBL|AVAILABLE|TORA)\D{0,12}(\d{3,5})\s*FT/,
       /(\d{3,5})\s*FT\D{0,12}(?:USABLE|AVBL|AVAILABLE)/
     ]){
       const m=raw.match(p);
       if(m){const v=Number(m[1]);if(Number.isFinite(v))usable=usable==null?v:Math.min(usable,v);}
     }
   }
 }
 return{state,usable,matched};
}
function effectiveLandingRunwayAssessment(){
 const published=runwayPublishedLength();
 const manualStatus=$("manualRunwayStatusOverride")?.value||"";
 const manualLen=valueOrNull("manualUsableLda");
 let state="OPEN",usable=published,source="PUBLISHED";
 if(currentNotams.status==="CHECKED"){
   state=currentNotams.runway_state||"OPEN";
   if(Number.isFinite(currentNotams.usable_length_ft))usable=currentNotams.usable_length_ft;
...
function updateLandingRunwayAssessment(){
 ...
 if(!r||!Number.isFinite(a.usable)){gate.textContent="NOT EVALUATED";note.textContent="Destination runway/usable length unavailable.";box.className="runway-gate warn";}
   ...
   if(currentNotams.status==="CHECKED"){
     ...
     gate.textContent="NOTAM NOT VERIFIED";
     note.textContent=`Runway length passes by ${margin} ft, but live NOTAM verification is incomplete.`;
...
async function refreshNotams(){
 ...
 try{
   ...
   currentNotams={status:"CHECKED",source:j.source||"FAA NMS",items,runway_state:parsed.state,usable_length_ft:parsed.usable,checked_at:j.checked_at||new Date().toISOString()};
   ...
   updateLandingRunwayAssessment();updateToldCard();
 }catch(e){
   ...
   $("notamList").textContent="Manual NOTAM verification required.";updateLandingRunwayAssessment();updateToldCard();
 }
}
...
async function evaluateTakeoff(){
 ...
 try{
   ...
   // The NMS scan is an automated review only. It does not replace a validated obstacle-clearance analysis.
   pending.push("OBSTACLE CLEARANCE ANALYSIS NOT PROVIDED");

   if(j.status==="GO"&&!pending.length){
     $("toStatus").textContent=climb.noLimitation?"GO • FIELD PASS / CLIMB NOT LIMITING":"GO • FIELD / CLIMB PASS";
     $("toStatus").className="ok";
   }else if(j.status==="GO"&&Number.isFinite(fieldWt)&&Number.isFinite(climb.limit)){
     $("toStatus").textContent=obs.status==="FLAGGED"?"PERFORMANCE PASS • OBSTACLE NOTAM REVIEW REQUIRED":"PERFORMANCE PASS • OBSTACLE CLEARANCE NOT EVALUATED";
     $("toStatus").className="warn";
   }else{
...
async function evaluateLanding(){
 ...
 try{
   ...
   else if(j.status==="GO"){$("landStatus").textContent="GO";$("landStatus").className="ok"}
   else{$("landStatus").textContent="PENDING • AFMS LANDING CLIMB SOURCE COVERAGE";$("landStatus").className="warn"}
...
async function startupCheck(){
 try{const r=await fetch(apiUrl("/api/diagnostics?_="+Date.now()),{cache:"no-store"}),j=await r.json();$("missionStatus").innerHTML=`<span class="${j.awc_metar?'ok':'bad'}">FlightOps server online — ${j.awc_metar?'weather connected':'weather unavailable'}</span>`}catch(e){$("missionStatus").innerHTML='<span class="bad">FlightOps server unavailable: '+e+'</span>'}
}
