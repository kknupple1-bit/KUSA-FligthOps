...
function makeEditableTripShareLink(record=null){
 ...
 const payload={v:1,kind:"editable-trip",generated:new Date().toISOString(),record:rec};
 return `${window.location.origin}${window.location.pathname}#trip=${b64urlEncodeUtf8(JSON.stringify(payload))}`;
}
async function shareEditableRecord(record){
 const url=makeEditableTripShareLink(record);
 const r=record||draftRecord();
 const label=`${r?.dep||"—"} → ${r?.dest||"—"} • ${r?.aircraft||"Aircraft"}`;
 try{
   if(navigator.share){await navigator.share({title:`KUSA FlightOps Editable Trip — ${label}`,text:"Open this KUSA FlightOps trip as an editable draft. Your copy will not alter the sender's original.",url});return}
   await navigator.clipboard.writeText(url);alert("Editable FlightOps trip link copied. Recipient can open it and import a working copy into Drafts.");
 }catch(e){if(e&&e.name!=="AbortError")prompt("Copy this editable FlightOps trip link:",url)}
}
...
function openSharedContentFromHash(){
 ...
 if(hash.startsWith("#trip=")){
   try{
     ...
     const r=p.record;
     if($("sharedToldTitle"))$("sharedToldTitle").textContent=`KUSA FlightOps — Editable Trip Transfer`;
...
async function startupCheck(manual=false){
 const expected="5.25.4",bs=$("startupBuildStatus"),bn=$("startupBuildNote"),ps=$("startupPerfStatus"),pn=$("startupPerfNote"),rs=$("startupRunwayStatus"),rn=$("startupRunwayNote"),ls=$("startupLiveStatus"),ln=$("startupLiveNote"),notice=$("startupDataNotice"),btn=$("refreshDataStatusBtn");
