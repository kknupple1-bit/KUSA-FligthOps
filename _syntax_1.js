...
function explicitSelectedRunwayClosure(raw,runwayId){
 ...
 const matches=[...raw.matchAll(/\b(?:RWY|RUNWAY)\s+([0-9]{1,2}[LCR]?(?:\s*\/\s*[0-9]{1,2}[LCR]?)?)\s+CLSD\b/g)];
 for(const m of matches){
   const listed=String(m[1]||"").split("/").map(normRunwayId);
   if(listed.includes(id))return true;
 }
 // Common explicit phrasing: RWY 12 AND RWY 30 CLSD
 const pair=[...raw.matchAll(/\bRWY\s+([0-9]{1,2}[LCR]?)\s+(?:AND|&)\s+RWY\s+([0-9]{1,2}[LCR]?)\s+CLSD\b/g)];
 return pair.some(m=>[normRunwayId(m[1]),normRunwayId(m[2])].includes(id));
}
function parseRunwayClosureFromNotams(items, runwayId){
 ...
 for(const n of items||[]){
   ...
   let localUsable=null;
   for(const p of [
     /(?:LDA|USABLE|AVBL|AVAILABLE|TORA)\D{0,12}(\d{3,5})\s*FT/,
     /(\d{3,5})\s*FT\D{0,12}(?:USABLE|AVBL|AVAILABLE)/
   ]){
     const m=raw.match(p);if(m){const v=Number(m[1]);if(Number.isFinite(v))localUsable=localUsable==null?v:Math.min(localUsable,v);}
   }
   if(Number.isFinite(localUsable)){
     usable=usable==null?localUsable:Math.min(usable,localUsable);
     if(state!=="CLOSED")state="RESTRICTED";
     restrictionMatches.push(raw);
   }else if(/\bRSTR\b|\bRESTRICTED\b/.test(raw)){
     if(state!=="CLOSED")state="RESTRICTED";
     restrictionMatches.push(raw);
   }else{
     // ILS/LOC/GP/light/procedure/obstruction notices are advisories unless
     // the NOTAM explicitly closes or shortens the selected runway.
...
function runwayImpactSummary(parsed,runwayId){
 ...
 if(parsed.state==="CLOSED")return`RWY ${id}: CLOSED by ${parsed.closureMatches.length} explicit runway-closure NOTAM${parsed.closureMatches.length===1?"":"s"}.`;
...
async function startupCheck(){
 const expected="5.23.1",bs=$("startupBuildStatus"),bn=$("startupBuildNote"),rs=$("startupRunwayStatus"),rn=$("startupRunwayNote"),ls=$("startupLiveStatus"),ln=$("startupLiveNote"),notice=$("startupDataNotice");
 try{
