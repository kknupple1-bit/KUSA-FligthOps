...
async function getNmsAccessToken(force=false){
  ...
  const r=await fetch(NMS_AUTH_URL,{
    ...
    headers:{
      ...
      "User-Agent":"KUSA-FlightOps/5.25.39"
    },
...
// v5.25.39: deterministic Falcon 50 S+20 nomograph delivery.
...
app.get("/api/data/f50_s20_nomograph_rev31",(req,res)=>{
  res.set("Cache-Control","no-store, no-cache, must-revalidate, proxy-revalidate");
  const p=path.join(__dirname,"public","data","f50_s20_nomograph_rev31.json");
  try{
    const raw=fs.readFileSync(p,"utf8");
    const parsed=JSON.parse(raw);
    return res.json(parsed);
  }catch(e){
    console.warn("F50 S20 nomograph static file unavailable; serving embedded verified fallback:",String(e?.message||e));
    return res.json(F50_S20_NOMOGRAPH_FALLBACK);
  }
});
...
app.get("/api/health",(req,res)=>res.json({ok:true,build:"5.25.39",platform:"GoDaddy Node.js",node:process.version,runway_airports_loaded:Object.keys(runwayDb).length,nms_environment:NMS_ENVIRONMENT,performance_models:["N33AP_F50_4","F900B_QRH1_REV02"]}));
