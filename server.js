...
async function getNmsAccessToken(force=false){
  ...
  const r=await fetch(NMS_AUTH_URL,{
    ...
    headers:{
      ...
      "User-Agent":"KUSA-FlightOps/5.25.33"
    },
...
app.get("/api/health",(req,res)=>res.json({ok:true,build:"5.25.33",platform:"GoDaddy Node.js",node:process.version,runway_airports_loaded:Object.keys(runwayDb).length,nms_environment:NMS_ENVIRONMENT,performance_models:["N33AP_F50_4","F900B_QRH1_REV02"]}));
